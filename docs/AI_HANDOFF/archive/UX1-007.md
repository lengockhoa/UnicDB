...
Add a gear icon to the `UnicDB.schemaTree` view title bar that opens VS Code's Settings UI
pre-filtered to the UnicDB extension's settings — the future hub for all
`contributes.configuration` entries.

## Target Files

- `package.json` — command `UnicDB.openSettings` (title `UnicDB: Open Settings`,
  icon `$(settings-gear)` — distinct from `UnicDB.openAiSettings`'s `$(gear)`); one
  `view/title` entry `when: view == UnicDB.schemaTree`, `group: navigation`;
  `onCommand:UnicDB.openSettings` activation line (guard filter from UX1-006).
- `src/extension.ts` — handler: `vscode.commands.executeCommand(
  "workbench.action.openSettings", "@ext:lengockhoa.UnicDB")`; register the command.
...
|---|------|-----------|----------|---------------------|
| 1 | happy | command opens settings filtered to the extension | `UnicDB.openSettings` invoked → `executeCommand` called with `workbench.action.openSettings` and `"@ext:lengockhoa.UnicDB"` | stubbed vscode.commands recording calls |
...
| 3 | edge B — structural wiring | package.json declares command + title entry | commands array contains `UnicDB.openSettings` with `$(settings-gear)`; `view/title` has the entry with exact `when: view == UnicDB.schemaTree` and `group: navigation` | module-level `pkgJson` in extension.test.ts |
| 4 | edge C — distinct icon namespace | no icon collision with AI settings entry | `UnicDB.openAiSettings` keeps `$(gear)`; `UnicDB.openSettings` uses `$(settings-gear)` — assert both in one test so a future icon swap can't silently collide | same |
| 5 | regression | command registration smoke | `extension.activate` registers `UnicDB.openSettings` (follows the "register đủ command" pattern, extension.test.ts:356) | existing activate harness |
...
- Consumes: `vscode.commands.executeCommand("workbench.action.openSettings", <query>)`
  VS Code built-in; publisher/name from package.json (`lengockhoa.UnicDB`).
- Produces: `UnicDB.openSettings` command id — UX1-004's user guide references it as the
  settings entry point; future setting contributions appear under the `UnicDB.*`
...
No runtime opening of `UnicDB.*` keys beyond the `@ext:` filter — `@ext:lengockhoa.UnicDB`
...
RED_OUTPUT: (reported confirmed in worktree — extension.test.ts UX1-007 showed 6 failing tests for the new UnicDB.openSettingsHub command + view/title menu entry)
