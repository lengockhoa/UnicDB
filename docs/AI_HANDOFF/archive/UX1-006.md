...
The `UnicDB.resultsPlacement` setting (enum `below|beside`, default `below`) plus the
...
- `package.json` — `contributes.configuration.UnicDB.resultsPlacement`: add `"top"` to
...
- `src/adapters/__tests__/bq04SurfaceGuard.test.ts` — extend `packageJsonDepsDiff`'s
  ...
  configuration-property-key pattern anchored to the `"UnicDB\.[a-zA-Z0-9.]+"\s*:\s*\{`
...
| 7 | regression | guard test 3 stays green with activationEvents + configuration lines | synthetic diff containing `+        "onCommand:UnicDB.openUserGuide",` and `+      "UnicDB.resultsPlacement": {` filters to EMPTY remaining diff; and a synthetic `+  "dependencies": {` line still FAILS the filter (guard still bites) | pure-function test over `packageJsonDepsDiff`-shaped input |
...
node -e "const p=require('./package.json'); const c=p.contributes.configuration.properties['UnicDB.resultsPlacement']; if(c.default!=='below'||!c.enum.includes('top')) { console.error('FAIL: default must stay below and enum must gain top'); process.exit(1);} console.log('OK: default=below, enum includes top');"
```
...
- [ ] Cases 1–8 pass; `UnicDB.resultsPlacement` enum is exactly `["below","beside","top"]`.
...
- Produces: (1) `readPlacementSetting(): "below" | "beside" | "top"` — later placement
  ...
  rely on `onCommand:` and `UnicDB.*` configuration-key lines being filtered so guard test 3
...
(`UnicDB.resultsPlacement` default `below` + CREATE-time moveEditorToBelowGroup) — the user
...
RED_OUTPUT:
```
RUN  v1.6.1 /Volumes/KHOA_EXTENAL/DOCKER_CREATE/UnicDB/.worktrees/ux1-006
 ...
 ❯ src/ui/__tests__/resultsPanel.test.ts  (58 tests | 2 failed) 424ms
   ❯ ResultsPanel — resultsPlacement (AI-001) > T3a. package.json manifest declares UnicDB.resultsPlacement (enum below|beside, default below)
...
The orchestrator's check uses
  p.contributes.configuration['UnicDB.resultsPlacement']
but the actual property path is
  p.contributes.configuration.properties['UnicDB.resultsPlacement']
...
- The orchestrator's quick `node -e` check uses a wrong JSON path
  (`configuration['UnicDB.resultsPlacement']` instead of
  `configuration.properties['UnicDB.resultsPlacement']`); the
