...
- `webview/styles.css` — `.UnicDB-chat-actions button svg {width:16px;height:16px;pointer-events:none}`, square icon-button sizing, hover affordance, `:focus-visible` ring; Send primary accent untouched.
...
| 9 | regression | grid toolbar icon pattern unaffected | `.UnicDB-btn svg` rule intact in styles.css; grid bundle tests untouched and green | current suite at HEAD |
...
- [ ] Send keeps `.UnicDB-chat-primary` accent (still visually primary).
...
- Consumes: existing DOM contract in `webview/aiChatPanelMain.ts` (button IDs `sendBtn`/`clearBtn`/`stopBtn`/`resumeBtn`/`regenerateBtn`/`attachBtn`; message `post()` types `send`/`resume_list` etc. — unchanged); `.UnicDB-btn svg` CSS pattern (styles.css:58-64) as visual precedent.
- Produces: `COMPOSER_ICONS` map in aiChatPanelMain.ts (`Record<ComposerButtonId, {svg: string; label: string}>`) — the icon+label pattern later webview toolbars can reuse; CSS class hook `.UnicDB-chat-actions button svg` for future icon sizing.
...
- **Resume picker Cancel stays text**: `renderResumePicker` builds its Cancel button outside `.UnicDB-chat-actions`; the "zero text labels" probe scopes to the 6 composer buttons only (by ID). `btnContaining(root, "Resume")` migrations landed in all owned + neighboring suites (aiChatPanelWebview.test.ts ×4, aiChatPanelWebviewTask002.test.ts ×1, aiChatPanelBundle.test.ts ×3); the task002 copy of `btnContaining` became unused and was removed (its webview.test.ts sibling stays — "Allow once"/"**Refuse**" permission-card locators still need text matching).
- **`#regenerateBtn` CSS rule removed**: the per-ID font-size/padding rule was dead weight on an icon tile; the shared `.UnicDB-chat-actions button` block now sizes/styles all six. The pinned chatLayoutCss contract test was migrated to accept the shared tile rule as the affordance source (per-ID override and `.UnicDB-chat-secondary` kept as accepted alternatives to not over-constrain).
- **`UnicDB-chat-secondary` class retained in markup** on resume/regen/stop (zero CSS rules target it now) — kept only as a semantic hook for future styling; removal would have touched the same template lines anyway, but the class costs nothing and preserves the existing chatLayoutCss fallback branch.
...
- SUMMARY: Composer toolbar is icon-only: new `COMPOSER_ICONS` map (svg+label per button id) + `composerIconSvg`/`iconButtonHtml` render helpers in `webview/aiChatPanelMain.ts`; all 6 buttons (resumeBtn/clearBtn/regenerateBtn/stopBtn/attachBtn/sendBtn) render a 16×16 currentColor inline SVG with `title` === `aria-label` from the map; `setBusy` re-asserts the attach title+aria-label pair on busy/vision flips; CSS turns `.UnicDB-chat-actions button` into 28×28 icon tiles with 16px svg, hover (`--UnicDB-input-hover-bg`), `:focus-visible` ring; obsolete `#regenerateBtn` per-ID rule removed; all text-based "Resume" locators migrated to `#resumeBtn` (bundle ×3, webview ×4, task002 ×1; unused task002 `btnContaining` helper removed; chatLayoutCss regenerate affordance contract migrated to accept the shared tile rule); CHANGELOG entry under [Unreleased].
...
- FILES_CHANGED:
  ...
  - webview/styles.css: `.UnicDB-chat-actions button` icon-tile rules (svg sizing, hover, focus-visible); removed `#regenerateBtn` rule
  ...
  - src/ui/__tests__/chatLayoutCss.test.ts: regenerate affordance contract accepts shared `.UnicDB-chat-actions button` tile rule
  - CHANGELOG.md: [Unreleased] entry for the icon-only composer toolbar
- TESTS_ADDED:
  - src/ui/__tests__/aiChatPanelBundle.test.ts: #AG1 (one svg per button), #AG2 (zero text labels), #AG3 (title === aria-label), #AG7 (all click handlers post), #AG8 (resume via #resumeBtn), #AG9 (.UnicDB-btn svg rule intact)
