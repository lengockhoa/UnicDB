...
Switch table and view tree nodes to `TreeItem.command = UnicDB.browseTableData` (double-click/Enter
...
- `src/ui/schemaTree.ts` — tables block (≈L390-408) and views block (≈L411-427) in
  `getCategoryChildren`: `command.command` `"UnicDB.copyQualifiedName"` → `"UnicDB.browseTableData"`,
...
- `package.json` — `contributes.commands` += `{ "command": "UnicDB.browseTableData", "title":
  "UnicDB: Browse Table Data", "category": "UnicDB" }`; `activationEvents` += `"onCommand:
  UnicDB.browseTableData"`. ADDITIVE ONLY — the file carries unrelated uncommitted edits.
...
|---|------|----------|----------|---------------------|
| 1 | regression-of-change happy | table node command | `getChildren(category tables)` node: `command.command === "UnicDB.browseTableData"`, `command.title === "Browse Data"`, `command.arguments[0].meta.schema/objectName/connection` populated, node still `collapsible: Collapsed` | adapter.listTables returns 1 table |
| 2 | regression-of-change happy | view node command | same assertions for views; routines node still `"UnicDB.copyQualifiedName"` | adapter.listViews/listRoutines stubs |
| 3 | edge wiring | extension registration | after `activate(ctx)`: `registeredCommands.has("UnicDB.browseTableData")` and invoking it does not throw | extension.test.ts harness |
| 4 | edge manifest | package.json | JSON parses; activationEvents contains `onCommand:UnicDB.browseTableData`; contributes.commands contains the entry with category UnicDB | read package.json from disk in test |
| 5 | edge boundary | connection nodes untouched | root connection node still `UnicDB.selectConnectionFromTree` with `[id]` args | schemaTree harness |
...
- TASK-001 — consumes `registerBrowseCommands`/`UnicDB.browseTableData` it produces.

## Interfaces

- Consumes: `registerBrowseCommands(deps: { mgr: ConnectionManager; runner: QueryRunner; panel: ResultsPanel }): void` and command id `UnicDB.browseTableData(node?: { meta?: { connection?: ConnectionConfig; schema?: string; objectName?: string } })` from TASK-001 (`src/ui/browseCommands.ts`). `UnicDBNode.command?: { command: string; title: string; arguments?: unknown[] }` (src/ui/schemaTree.ts).
...
simplest correct shape: `const n: UnicDBNode = { ...fields, meta }; n.command = { command:
"UnicDB.browseTableData", title: "Browse Data", arguments: [n] }; return n;`. Passing the whole node
...
SUMMARY: Wired `UnicDB.browseTableData` for double-click/Enter on table + view schemaTree nodes (routines + connection nodes unchanged). `src/ui/schemaTree.ts` builds the UnicDBNode first then attaches `command = { command: "UnicDB.browseTableData", title: "Browse Data", arguments: [n] }` in the tables + views blocks. `src/extension.ts` imports `registerBrowseCommands` from `./ui/browseCommands` and calls it right after `ResultsPanel` is constructed (panel/runner/mgr all available there) with `{ mgr, runner, panel }`. `package.json` gains one `contributes.commands` entry (title "UnicDB: Browse Table Data", category "UnicDB") + one `activationEvents` entry (`onCommand:UnicDB.browseTableData`) — purely additive (the only "removed" byte is the trailing comma needed for valid JSON after the new activationEvents row).
TEST_PLAN_FOLLOWED: task §4 (all 5 cases implemented; case 5 = regression-protection PASSES against baseline)
FILES_CHANGED:
  - src/ui/schemaTree.ts: tables + views blocks in getCategoryChildren — build UnicDBNode first then attach command with whole node as arg
  - src/extension.ts: import registerBrowseCommands; call right after `panel.setExtensionUri(...)`/`context.subscriptions.push(panel)`
  - package.json: +1 activationEvent ("onCommand:UnicDB.browseTableData") +1 contributes.commands entry
  ...
  - src/extension.test.ts: +4 cases (case 3 registration, case 3 palette no-throw, case 4 contributes entry, case 4 activationEvent) in new describe "TASK-002 — UnicDB.browseTableData extension wiring"
TESTS_ADDED:
  - src/ui/__tests__/schemaTree.test.ts: TASK-002 browse gesture wiring describe (3 cases)
  - src/extension.test.ts: TASK-002 — UnicDB.browseTableData extension wiring describe (4 cases)
VERIFICATION:
  ...
  output_excerpt: |
    RED (6 new-test failures captured before src edits):
      RUN  v1.6.1 /Volumes/KHOA_EXTENAL/DOCKER_CREATE/UnicDB/.worktrees/task-002
       FAIL  src/extension.test.ts > TASK-002 — UnicDB.browseTableData extension wiring > registers UnicDB.browseTableData handler trong activate() (case 3)
       AssertionError: expected false to be true  // command not registered yet
       FAIL  src/extension.test.ts > TASK-002 — UnicDB.browseTableData extension wiring > invoking UnicDB.browseTableData handler does not throw when invoked from palette (no arg) (case 3)
       AssertionError: expected undefined not to be undefined  // fn = registeredCommands.get(...)
       FAIL  src/extension.test.ts > TASK-002 — UnicDB.browseTableData extension wiring > package.json contributes.commands has UnicDB.browseTableData entry with category UnicDB (case 4)
       AssertionError: expected undefined not to be undefined  // entry not in manifest yet
       FAIL  src/extension.test.ts > TASK-002 — UnicDB.browseTableData extension wiring > package.json activationEvents has onCommand:UnicDB.browseTableData (case 4)
       AssertionError: expected [ 'onLanguage:sql', …(20) ] to include 'onCommand:UnicDB.browseTableData'
       FAIL  src/ui/__tests__/schemaTree.test.ts > SchemaTreeProvider — TASK-002 browse gesture wiring > table node command = UnicDB.browseTableData 'Browse Data' with arguments[0]=node (case 1)
       AssertionError: expected 'UnicDB.copyQualifiedName' to be 'UnicDB.browseTableData'  // schemaTree tables block unchanged
       FAIL  src/ui/__tests__/schemaTree.test.ts > SchemaTreeProvider — TASK-002 browse gesture wiring > view node command = UnicDB.browseTableData 'Browse Data' with arguments[0]=node (case 2)
       AssertionError: expected 'UnicDB.copyQualifiedName' to be 'UnicDB.browseTableData'  // schemaTree views block unchanged
...
FINDINGS:
  ...
  minor:
    - package.json:189-193 — add `"icon": "$(database)"` (or a similar DataGrip-style icon matching the "Browse Data" semantics) to the `UnicDB.browseTableData` command entry.
...
SUMMARY: Fix the single critical finding from R4.5 Reviewer Verdict: add `"icon": "$(preview)"` to the `UnicDB.browseTableData` contributes.commands entry at package.json:190 (the only command missing an icon — scaffold.test.ts:126 loops every entry asserting `cmd.icon` matches `/^\$\(/`). No other code/test edits; meets icon regex `^\$\(`.
TEST_PLAN_FOLLOWED: N/A — single-line manifest patch; test suite re-run as scoped proof.
FILES_CHANGED:
  - package.json: UnicDB.browseTableData entry at L190 — added `"icon": "$(preview)"` (purely additive, no removed lines)
...
NOTES: Fix-002 added icon="$(preview)" to the UnicDB.browseTableData command entry at package.json:190. Scaffold icon assertion loop now passes; no other regressions. Fix fully resolves the critical_block from R4.
