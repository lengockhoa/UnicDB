...
- `webview/aiChatPanelMain.ts` — (a) new `appendThinking()` / `removeThinking()` pair
  rendering a `.UnicDB-chat-thinking-row` (spinner glyph + text "AI is thinking…"), invoked
...
- `webview/styles.css` — append chat-region rules: `.UnicDB-chat-thinking-row` (spinner via
  CSS animation) + `overflow-wrap: anywhere` on `.UnicDB-chat-bubble`. Modify NO other
  selector blocks (`.UnicDB-chat-*` region shared with UX1-008 — wave 2, no overlap since
  UX1-008 is wave 1).
...
|---|------|-----------|----------|---------------------|
| 1 | happy | send shows thinking row; first delta removes it | after send, thread contains `.UnicDB-chat-thinking-row` with text `AI is thinking…`; after first `{type:"delta"}` post the row is gone | bundle loaded in jsdom; stubbed acquireVsCodeApi (existing bundle harness) |
| 2 | happy | closed fence mid-stream renders boxed code with copy button | send delta chunks ending with a closed ```` ``` ```` fence → bubble contains `pre.UnicDB-md-code` + `button.UnicDB-md-copy`; clicking the button copies the raw code (data-raw un-escape path) | bundle harness; clipboard stub |
| 3 | edge A — unterminated fence | open fence mid-stream stays plain text | after a delta containing ```` ```sql\nSELECT 1 ```` (no closing fence) the bubble contains NO `pre.UnicDB-md-code`; after the closing-fence delta it does (exactly once) | bundle harness |
...
| 6 | edge C — re-render idempotence | repeated deltas over a closed fence do not duplicate copy buttons | after 3 subsequent deltas, bubble contains exactly one `.UnicDB-md-copy` per fenced block | bundle harness |
...
| 8 | regression | truncation contract | `ruleBody('.UnicDB-chat-bubble')` contains `overflow-wrap: anywhere` (chatLayoutCss pattern) | styles.css from disk |
...
- Produces: `.UnicDB-chat-thinking-row` DOM contract + streaming re-render on closed fence —
...
P1's description of R11 as "add a copy button" is already satisfied — `UnicDB-md-copy` ships
