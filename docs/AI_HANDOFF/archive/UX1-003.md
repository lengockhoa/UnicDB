...
UnicDB Console opens pre-filled with typed INSERT template statements the user reviews and
runs manually ("tạo trên console cho tôi 1 bộ query để tôi tự chạy"). Keep the command id
`UnicDB.generateSampleData`; retitle to "Insert Sample Data…".

## Target Files

- `package.json` — retitle `UnicDB.generateSampleData` to `Insert Sample Data…` (icon
  stays `$(file-text)`); activationEvents line already exists.
- `src/ui/tableCommands.ts` — rewire the `UnicDB.generateSampleData` handler (:355): node
...
| 2 | happy | menu retitle | package.json `UnicDB.generateSampleData` title is `Insert Sample Data…`; command id unchanged (activationEvents + menu entries untouched) | module-level `pkgJson` assertion |
...
console templates with manual execution. Command id stays `UnicDB.generateSampleData` to
