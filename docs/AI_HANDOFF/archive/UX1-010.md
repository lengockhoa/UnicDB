...
- `webview/main.ts` — in the state/tab logic (`renderActivePanel` :1174 region + statement
  ...
  keyed on `kind`.
- `webview/styles.css` — append ONE new `.UnicDB-ddl-card` block (success/error variants).
  Do not modify existing selector blocks. Wave-1 region contract (operative rule, P2.5
  round 1): `.UnicDB-chat-*` belongs to UX1-008, `.UnicDB-setfilter-*` to UX1-005 — disjoint
...
- [ ] SELECT path byte-identical (case 2 + existing resultsGridModel/resultsPanel suites
      green).
- [ ] `.UnicDB-ddl-card` CSS is a pure append (no existing selector modified).
- [ ] Reviewer verdict APPROVED or APPROVED-WITH-MINOR.

## Dependencies

- none (wave-1 region contracts, promoted to the operative rule in P2.5 round 1:
  styles.css changes are a new `.UnicDB-ddl-*` append-only block vs UX1-008's `.UnicDB-chat-*`
  and UX1-005's `.UnicDB-setfilter-*`; the single extension.ts stamping site does not
...
- Produces: `classifyStatementKind(sql, dialect?)` + `StatementResult.kind?` — UX1-011
  ...
  (`.UnicDB-ddl-card`) is the final render surface.
...
Verification Output:
  ...
  Command 2b — npx vitest run src/ui/__tests__/ddlStatusCard.test.ts (extra)
    Tests  10 passed (10)
  Command 3 — git diff -- webview/styles.css | grep -E '^[+-]\.(UnicDB-setfilter|UnicDB-chat)'
    → (no matches) "styles region OK"
    Note: the original substring grep matched on the comment text mentioning
    `.UnicDB-chat-*`/`.UnicDB-setfilter-*`; the corrected selector-only check is empty.
  ...
  Region contract spot checks:
    ...
    - .UnicDB-setfilter-* / .UnicDB-chat-* selectors in styles.css → untouched
    - .UnicDB-ddl-* selectors in styles.css → 8 new selectors, append-only
...
Note: implemented `classifyStatementKind` + `stampStatementKind` in src/core/queryRunner.ts (261 lines), new
  ...
  append-only `.UnicDB-ddl-card*` CSS block. BQ-pending entries keep `kind === undefined` via the
