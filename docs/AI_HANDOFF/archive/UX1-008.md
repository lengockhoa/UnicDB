...
- `webview/styles.css` — ONLY the chat-region blocks: `.UnicDB-chat-bubble` (:918),
  `.UnicDB-chat-assistant` (:930), `.UnicDB-chat-assistant.UnicDB-chat-streaming` (:935),
  `.UnicDB-chat-thread` (:909), `.UnicDB-chat-caret` (:1490). Do NOT touch
  `.UnicDB-setfilter-*` (UX1-005) or add `.UnicDB-ddl-*` (UX1-010) — region exclusivity per
  PLAN §2.
...
|---|------|-----------|----------|---------------------|
| 1 | happy | streaming bubble cannot collapse to one char per line | `ruleBody('.UnicDB-chat-bubble')` (or the streaming compound rule) declares a positive `min-height` (e.g. `1lh`) AND `width: fit-content` AND retains `max-width` ≤ 95% | styles.css read from disk |
| 2 | happy | caret no longer forces its own line box | `.UnicDB-chat-caret` rule no longer contains `display: inline-block` (replaced with `display: inline` or equivalent) | same |
...
| 4 | edge B — boundary padding | assistant bubbles off the left edge | `.UnicDB-chat-assistant` (or `.UnicDB-chat-thread`) declares `padding-left ≥ 12px` (or thread margin-left ≥ 8px) | same |
| 5 | edge C — user bubble unaffected | user bubbles stay right-aligned with their own padding | `.UnicDB-chat-user` rule still declares `align-self: flex-end` | same |
...
- [ ] Only chat-region CSS changed; `git diff -- webview/styles.css` contains no
      `.UnicDB-setfilter` / `.UnicDB-ddl` / `.UnicDB-md` hunks (wave exclusivity with UX1-005,
      UX1-010).
...
- none (wave-1 region contract — promoted to the operative rule by P2.5 round 1, retiring
  ...
  `.UnicDB-chat-*` selectors; UX1-005 owns `.UnicDB-setfilter-*`, UX1-010 appends only the new
  `.UnicDB-ddl-*` block — disjoint selector families, P3 merge conflict-free.)

## Interfaces

- Consumes: `.UnicDB-chat-*` CSS region (webview/styles.css:887-1000, 1472-1500);
...
code-derived: `.UnicDB-chat-bubble` uses `white-space: pre-wrap` and the caret is
...
- **R9 case 1** — `.UnicDB-chat-bubble` now declares `width: fit-content` and `min-height: 1lh`.
...
- **R9 case 2** — `.UnicDB-chat-caret` switched from `display: inline-block` to
...
- **R10 case 4** — `.UnicDB-chat-assistant` now declares `padding-left: 12px`, lifting
...
RED_OUTPUT: |
  ❯ src/ui/__tests__/chatLayoutCss.test.ts  (30 tests | 3 failed)
    ❯ R9 case 1: .UnicDB-chat-bubble declares min-height + width:fit-content and retains max-width<=95%
      → .UnicDB-chat-bubble must declare a positive min-height (e.g. 1lh / 16px): expected false to be true
    ❯ R9 case 2: .UnicDB-chat-caret no longer uses display:inline-block (was the one-char-per-line cause)
      → .UnicDB-chat-caret must NOT declare display:inline-block (forces own line box, garbles streaming text): expected true to be false
    ❯ R10 case 4: assistant bubbles are NOT flush against the left border (padding-left>=12px or thread margin-left>=8px)
      → either .UnicDB-chat-assistant padding-left>=12px OR .UnicDB-chat-thread margin-left>=8px must hold (R10): expected false to be true
...
Verification Output: |
  ...
  # 2. region contract (no edits outside .UnicDB-chat-*)
  $ git diff -- webview/styles.css | grep -E '^[+-].*\.(UnicDB-setfilter|UnicDB-ddl)' && { echo "REGION VIOLATION"; exit 1; } || echo "region OK"
  region OK
...
Status: PASS
Note: Region contract honored (only .UnicDB-chat-bubble, .UnicDB-chat-assistant, .UnicDB-chat-caret
...
NOTES: Region contract honored — UX1-008 hunks confined to .UnicDB-chat-bubble (:933-934), .UnicDB-chat-assistant (:951), .UnicDB-chat-caret (:1511); no collision with wave-2 .UnicDB-chat-thinking-row (:1730, new selector) or .UnicDB-md-* (:1475).
