...
- `src/ui/tableCommands.ts` (modify) — register `UnicDB.postmanPayload` inside
...
- `package.json` (modify) — `contributes.commands` += `UnicDB.postmanPayload` ("UnicDB: Postman
  Payload", category UnicDB, icon `$(copy)`); `activationEvents` += entry; `menus.view/item/
  context` += entry when `view == UnicDB.schemaTree && (viewItem == table || viewItem == view ||
...
| 9 | wiring | menu + registration | context-menu entry on table/view/viewItem routine; `registeredCommands.has("UnicDB.postmanPayload")` | extension.test harness or tableCommands harness + package.json fs read |
...
- Produces: `buildPostmanPayload(schema: string, name: string, columns: string[]): string` and
  ...
  PostgresAdapter; command id `UnicDB.postmanPayload(node?: { meta?: … })`.
...
SUMMARY: Implemented `buildPostmanPayload` + `jsKey` pure builder in new `src/ui/postmanPayload.ts`, added `listRoutineParams` to `DbAdapter` (Postgres pg_proc implementation + mysql/mssql NotImplementedError stubs), registered `UnicDB.postmanPayload` command in `registerTableCommands` (table/view → `listColumns`, routine → `listRoutineParams`), and added the command + menu entry to `package.json`.
...
FILES_CHANGED:
  ...
  - `src/ui/tableCommands.ts`: added `postmanPayload: "Postman Payload"` to `COMMAND_TITLE`; registered `UnicDB.postmanPayload` command at end of `registerTableCommands` (after `UnicDB.createSchema`); resolves node via `resolveTableNode` (reused guard pattern), dispatches by `contextValue === "routine"` to `listRoutineParams` else `listColumns`; driver guard reuses `guardPostgres` → info message on mysql/mssql; clipboard + status bar on success; error message on failure.
  - `package.json`: ADDITIVE — `contributes.commands` += `{ command: "UnicDB.postmanPayload", title: "UnicDB: Postman Payload", category: "UnicDB", icon: "$(copy)" }`; `activationEvents` += `onCommand:UnicDB.postmanPayload`; `contributes.menus["view/item/context"]` += entry with `when: "view == UnicDB.schemaTree && (viewItem == table || viewItem == view || viewItem == routine)"`.
...
TESTS_ADDED:
  ...
  - `src/ui/__tests__/tableCommands.test.ts`: describe "tableCommands — TASK-008 UnicDB.postmanPayload" (5 tests: case #2 view, case #3 routine, case #6/#6b mysql+mssql, case #7 empty, case #9 wiring).
...
NOTES: Parameterization verified — $1/$2 bind asserted, no injection. package.json slice is clean and purely additive (icon `$(copy)`, activationEvent, menu when-clause); the UnicDB.browseTableData missing-icon scaffold regression belongs to TASK-002, not this task. Builder/jsKey/driver-guard/mysql+mssql stubs all correct.
...
RED_OUTPUT: RED cannot be re-run against the old code from inside this worktree — the previous-round test harness pinned canned mock rows that bypassed real-SQL parsing, so the old green run was a false green. Reviewer explicitly verified the defect on real PG 16.15. Two pieces of post-fix evidence confirm the defect's scope:
  ...
  For full postgreSQL-on-real-DB coverage the reviewer recommended a `UnicDB_IT=1` case in `src/adapters/__tests__/ddl.integration.test.ts`; PG was unreachable in this worktree (`nc -z 127.0.0.1 5433` → `PG_PORT_CLOSED`) so that case cannot be authored here. Documented as a follow-up; the unit-level INOUT + SQL-text assertion cover the same defect surface.
TESTS_ADDED:
...
VERIFICATION:
  ...
  output_excerpt: |
    ...
    Test Files  3 passed (3)
         Tests  44 passed (44)
ISSUES: UnicDB_IT integration case for `listRoutineParams` against real PG cannot be authored from this worktree (PG_PORT_CLOSED) — follow-up for a runner with PG access. Reviewer-flagged minors (`p.name ?? ""` silent drop in postmanPayload + duplicate pre-check in tableCommands) intentionally left untouched (out of file ownership scope; assigned to fix-005 / a future minor round).
HANDOFF_TO_REVIEWER: yes
...
FIX_VERIFICATION:
  ...
  - Reviewer re-verified empirically on the real instance (UnicDB-postgres, PG 16.15 — container was stopped, reviewer started it, ran shipped SQL + old SQL, then stopped it again): named all-IN (p_user_id/p_amount), unnamed positional (null/null), no-arg ([]), INOUT mixed(a, INOUT b) → all 4 shapes correct. Old SQL confirmed defective: [] for all-IN shapes; [(null,null),(a,integer)] misalignment for INOUT. This closes the executor's PG_PORT_CLOSED gap — no UnicDB_IT case needed to unblock.
FINDINGS:
  critical: none
  important: none
  minor:
    ...
    - ddl.integration.test.ts — permanent UnicDB_IT=1 case for listRoutineParams still absent (PG unreachable in fix worktree; defect surface now covered by SQL-text tripwires + reviewer's real-PG run). Follow-up only.
