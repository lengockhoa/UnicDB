...
## Spec
**package.json** — activationEvents onCommand for each; commands (category UnicDB): `UnicDB.newTable` "New Table…", `UnicDB.modifyTable` "Modify Table…", `UnicDB.copyCreateDdl` "Copy CREATE DDL", `UnicDB.generateSampleData` "Generate Sample Data…", `UnicDB.analyzeTable` "Analyze Table", `UnicDB.vacuumTable` "Vacuum Table". `menus["view/item/context"]` (group "UnicDB"): newTable when `view == UnicDB.schemaTree && (viewItem == schema || viewItem == category || viewItem == table)`; other 5 when `view == UnicDB.schemaTree && viewItem == table`.
...
- **UnicDB.modifyTable** — table nodes only; loadSpec runs both INTROSPECT SQLs via adapter.runQuery → `rowsToSpec`; runDdl executes diff statements joined `\n` → refresh + reveal + "Modified ${schema}.${table}".
- **revealTableNode** (`schemaTree.ts`): `revealTableNode(treeView, conn, schema, table): Promise<void>` + `SchemaTreeProvider.findTableNode(conn, schema, table): Promise<UnicDBNode | null>` (reuse caches/getCategoryChildren path; null when absent) → `treeView.reveal(node, {select:true, expand:false})` catch-ignore.
- **UnicDB.copyCreateDdl** — introspect → `generateCreateTable(spec)` → `vscode.env.clipboard.writeText` + status bar `UnicDB: DDL copied` 2s (pattern 628-631).
- **UnicDB.generateSampleData** — `showInputBox({prompt:"Number of rows", value:"10"})`; undefined → abort; NaN/negative → Information "Enter a positive number"; clamp `max(0,min(1000,n))`; introspect → `generateSampleInserts` → `openTextDocument({language:"sql", content})` + `showTextDocument`.
- **UnicDB.analyzeTable / vacuumTable** — `runQuery('ANALYZE ' + qualified)` / `runQuery('VACUUM ANALYZE ' + qualified)`, qualified = `quoteIdent(schema)+"."+quoteIdent(table)`; success → "<schema>.<table> analyzed"/"vacuumed".
...
- Produces: `generateSampleInserts(spec,n):string`; `revealTableNode(treeView,conn,schema,table):Promise<void>` + `SchemaTreeProvider.findTableNode(conn,schema,table):Promise<UnicDBNode|null>`; commands `UnicDB.newTable|modifyTable|copyCreateDdl|generateSampleData|analyzeTable|vacuumTable`.
...
VERIFICATION:
  ...
  output_excerpt: |
    ...
    Test Files  4 passed (4)
    Tests  96 passed (96)
  command: UnicDB_IT=1 UnicDB_PG_HOST=127.0.0.1 UnicDB_PG_PORT=5433 npx vitest run -c vitest.integration.config.ts src/adapters/__tests__/ddl.integration.test.ts
