...
## Goal
detectOmp(): locate the binary, read its version, compare against MIN_OMP_VERSION, return the decision to use omp or fall back to builtin — with install/update hint messages for the UI.

## Target Files
- `src/ai/omp/detect.ts` (new)
- `src/ai/omp/__tests__/detect.test.ts` (new)
...
export function compareVersions(a: string, b: string): number  // semantic compare "17.0.1" vs "17.10.0" as numeric per segment; non-numeric tail ignored
export async function detectOmp(execFn?: (cmd: string) => Promise<string>): Promise<OmpDetection>
export interface OmpDetection {
  available: boolean   // binary ran and returned a version
...
```
- execFn injectable (default: promisified exec via lazy import). Commands run: `which omp` (or the opts path), `<omp> --version`.
- No vscode import; runs nothing real when execFn is provided (tests are pure).
- Spawn error (ENOENT) → available=false, reason "not-installed" — does NOT throw.

## Test Cases
| # | Type | Name | Expected |
|---|------|-----|----------|
| 1 | happy | execFn returns path + "omp/18.0.1" | available, ok, version "18.0.1", path set |
| 2 | edge (missing) | execFn rejects ENOENT | available=false, reason "not-installed", does not throw |
| 3 | edge (old) | "omp/16.9.0" | ok=false, reason "version-too-old" |
| 4 | edge (garbage) | output cannot be parsed | reason "version-unknown", ok=false |
| 5 | unit | compareVersions "17.0.1" vs "17.10.0" | -1 (numeric, not string-compare); "18.0.0" vs "17.99.99" = 1; equal = 0 |
| 6 | edge (non-numeric tail) | "18.0.1-beta.2" | parses 18.0.1, comparison stable |
...
## Acceptance
- [x] 6 tests PASS RED→GREEN (real output)
- [x] No vscode import; no real exec when execFn provided
...
VERIFICATION:
  ...
  RED_OUTPUT: |
    ...
    Error: Failed to load url ../detect (resolved id: ../detect) in /Volumes/KHOA_EXTENAL/DOCKER_CREATE/UnicDB/src/ai/omp/__tests__/detect.test.ts. Does the file exist?
