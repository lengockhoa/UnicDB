...
existing post-run seam: DDL → full refresh (exact `UnicDB.refreshSchema` semantics), DML →
...
- `src/extension.ts` — replace `invalidateAfterSchemaDdl`'s body (:881) to dispatch on
  ...
  + sqlSemanticTokens.refresh() + tree.refresh()` (identical to the `UnicDB.refreshSchema`
  ...
  Do NOT touch the `UnicDB.refreshSchema` command registration itself.
...
- [ ] Full-refresh semantics byte-match the `UnicDB.refreshSchema` command body (case 1
