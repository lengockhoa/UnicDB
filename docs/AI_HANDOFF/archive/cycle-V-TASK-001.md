...
so UnicDB adds dialect scopes the built-in grammar misses (`ILIKE`, `RETURNING`, `TOP`,
`OFFSET … FETCH`, `[bracket]` / `` `backtick` `` quoted identifiers) without replacing —
and therefore without regressing — the built-in grammar.

## Target Files

- `syntaxes/UnicDB-sql-injection.tmLanguage.json` **(new)** — the injection grammar. Root
  keys: `scopeName: "source.sql.UnicDB"`, `injectionSelector: "L:source.sql"`, `patterns: []`.
- `package.json` — add `contributes.grammars` (one entry, `injectTo: ["source.sql"]`,
  `path: "./syntaxes/UnicDB-sql-injection.tmLanguage.json"`, `scopeName: "source.sql.UnicDB"`).
...
|---|------|----------|----------|---------------------|
| 1 | unit (happy) | `package.json contributes a grammar injected into source.sql` | `contributes.grammars` has length ≥ 1; entry `.injectTo` contains `"source.sql"`; `.scopeName === "source.sql.UnicDB"` | read `package.json` from disk |
...
| 3 | unit (happy) | `grammar declares at least the UnicDB dialect keywords` | joined `match` strings contain `ILIKE`, `RETURNING`, `TOP`, `FETCH` | parsed grammar |
...
- [ ] `syntaxes/UnicDB-sql-injection.tmLanguage.json` exists and `JSON.parse` succeeds.
...
- Consumes: (none)
- Produces:
  - grammar scope name `source.sql.UnicDB` — TASK-002's semantic token types layer on top of
  ...
  - Token scope names TASK-003's webview tokenizer mirrors for visual consistency:
    `keyword.other.UnicDB.sql`, `string.quoted.single.sql`, `constant.numeric.sql`,
    `comment.line.double-dash.sql`, `entity.name.function.UnicDB.sql`.
...
```
 FAIL  src/__tests__/sqlGrammar.test.ts > UnicDB sql injection grammar (TASK-001) > package.json contributes a grammar injected into source.sql
AssertionError: contributes.grammars must have at least one entry: expected 0 to be greater than or equal to 1
 ...
 FAIL  src/__tests__/sqlGrammar.test.ts > UnicDB sql injection grammar (TASK-001) > grammar file exists at the contributed path and parses as JSON
TypeError: Cannot read properties of undefined (reading 'path')
 ❯ src/__tests__/sqlGrammar.test.ts:66:18
   66|     expect(entry.path).toBeTruthy();

 FAIL  src/__tests__/sqlGrammar.test.ts > UnicDB sql injection grammar (TASK-001) > grammar declares at least the UnicDB dialect keywords
Error: ENOENT: no such file or directory, open '.../syntaxes/UnicDB-sql-injection.tmLanguage.json'
 ❯ readJson src/__tests__/sqlGrammar.test.ts:20:18

 FAIL  src/__tests__/sqlGrammar.test.ts > UnicDB sql injection grammar (TASK-001) > no pattern matches the empty string
Error: ENOENT: no such file or directory, open '.../syntaxes/UnicDB-sql-injection.tmLanguage.json'
...
dialect keywords are therefore matched in their conventional uppercase form. `entity.name.function.UnicDB.sql`
...
NOTES: Clean delivery. Grammar has 8 non-empty patterns covering all required UnicDB dialect tokens; regex safety confirmed; no contributes.languages re-declaration; vscodeignore correctly ships syntaxes/. All 6 test cases assert real behavior.
