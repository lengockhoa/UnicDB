...
- Three contributed commands: `vsdb.ai.showPolicy`, `vsdb.ai.exportTrace`, and `vsdb.ai.clearTrace`. Export uses VS Code's save dialog and `workspace.fs.writeFile`; a policy denied by distrust, invalid/migrated configuration, or invalid resolver state blocks export before file selection or write.
...
1. **Central decision, fail closed without contradicting engine resolution.** Add a VS Code-free `resolvePolicy(input): EffectivePolicy` that accepts workspace trust, the raw user-configured engine value read from `vscode.workspace.getConfiguration("vsdb").get("ai.engine", "builtin")`, and the already-resolved `EngineChoice` from `resolveEngine()`. Validate the configured value only as known preference vocabulary (`"builtin" | "omp"`); derive the effective provider exclusively from `EngineChoice.engine`. This is deliberately orthogonal because `resolveEngine()` selects OMP whenever detection succeeds, including a valid configured `builtin` default. Thus trusted `{ configuredEngine: "builtin", resolvedEngine: "omp" }` is a valid OMP posture that admits declared capabilities, not a conflict. Untrusted workspaces, unsupported/migrated configured values, or a missing/invalid resolver result deny sensitive context/tools and audit export with a concrete notice. A single path predicate denies credential/config locations such as `.env`, `.git/**`, and `.vscode/vsdb-ai-config.yml` before workspace context is admitted.
...
| edge — path | `policy excluded path predicate blocks credential/config locations` | `.env`, `.git/config`, and `.vscode/vsdb-ai-config.yml` are excluded; a normal workspace source path remains eligible. |
...
- Keep user-facing command names under `vsdb.ai.*`; use concise English notices consistent with existing `VSDB:` messages.
