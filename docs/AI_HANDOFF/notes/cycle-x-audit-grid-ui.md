...
- `buildPagedQuery`/`buildPagedQueryTerms` — mssql `ORDER BY (SELECT NULL)` placeholder, tiebreaker dedup, alias disjointness (`vsdb_page`/`vsdb_sort`/`vsdb_sub`/`vsdb_distinct`). Clean.
