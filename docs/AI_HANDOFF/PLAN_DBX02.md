...
| unit | Hover and definition resolve table column and FK target | Hover includes concrete type/target; definition points at `vsdb-sql-catalog:` whose registered provider returns typed table/column/FK metadata |
...
| integration-style smoke | Activation registers language and catalog-document providers once | SQL/file providers and one `vsdb-sql-catalog` text-document provider are registered; navigation-populated URI content is retrievable; absent APIs remain safe no-ops |
...
- [ ] TASK-DBX02-005 wires all language providers plus the `vsdb-sql-catalog` text-document provider to the one activation cache and proves registrations/content retrieval in `src/extension.test.ts`.
