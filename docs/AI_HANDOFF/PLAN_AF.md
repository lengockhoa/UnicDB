...
2. **Schema tree expansion + DDL viewer** — tree gets `indexes`/`constraints`/`triggers` categories under each table + `sequences` schema-level category + row counts in node descriptions; "Open DDL" context-menu action on table/view/routine/trigger nodes opening a read-only `vsdb-ddl:` virtual document.
...
- **DDL viewer as a `TextDocumentContentProvider`** on a `vsdb-ddl:` URI scheme: native editor, SQL language id, read-only, refresh command invalidates the per-URI cache. Alternative rejected: another webview — duplicate styling/scroll/copy work for zero gain.
