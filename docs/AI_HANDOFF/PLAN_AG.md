...
2. Icon rendering: "Inline SVG theo pattern .vsdb-btn sẵn có (Recommended)" — inline SVG 16×16 `currentColor` copying the existing `.vsdb-btn svg` pattern (styles.css:58-64, used by the data grid toolbar). No icon-font dependency.
...
1. `webview/aiChatPanelMain.ts` — `renderInitial()` markup: replace the 5 text labels with inline SVG icons (send, clear, regenerate, stop, resume), every icon button gets `title` + `aria-label` from one shared label-constant map (label text lives once, feeds tooltips). Keep: button IDs, classes, event wiring, hidden/disabled state logic, attach `+` behavior (only ensure its tooltip matches the shared pattern). `Send` keeps `.vsdb-chat-primary` accent, becomes icon.
2. `webview/styles.css` — square icon-button sizing for `.vsdb-chat-actions button` (icon 16×16, `pointer-events:none` on svg, hover affordance matching `.vsdb-btn:hover`), Send accent preserved, `:focus-visible` ring for keyboard users.
...
**Out of scope:** other panels' toolbars (grid/console/forms — they already use `.vsdb-btn` icons or are separate panels), jumpLatest, markdown code Copy buttons, permission-card buttons, attach pipeline logic (the file-input opening flow), any host-side (`src/ui/aiChatPanel.ts`) change, engine/settings forms.
...
- **Keep every existing hook intact**: button IDs, `.vsdb-chat-secondary`/`-primary`/`-attach-btn` classes, disabled/hidden state logic (renderBusyState :425-436, regenerate enable/disable :519-528, resume disabled while busy), event listeners — none change. Only inner content + tooltip attributes change.
- **CSS**: `.vsdb-chat-actions button svg {width:16px;height:16px;pointer-events:none}` + square min-width so icon buttons look uniform; `:focus-visible` ring; Send keeps its primary accent via existing class.
- **Send as icon (paper-plane)**: still `.vsdb-chat-primary`, visually distinct by accent color alone — matches the user's "chuyên nghiệp" ask while staying minimal.
...
| CSS icon sizing | svg 16×16 + pointer-events:none + hover affordance in styles.css | jsdom bundle-eval: composer buttons expose no visible text content | `:focus-visible` ring rule exists in styles.css | grid `.vsdb-btn svg` pattern users (grid toolbar) unaffected |
