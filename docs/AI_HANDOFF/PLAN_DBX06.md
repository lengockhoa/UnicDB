...
| edge / capability | tableDdl denied | `tableDdl: false` shows exactly `UnicDB: Rename Table is not supported by this connection's database.` with no panel, lookup, or DDL side effect. |
