...
Turn cycle AD's "export config + print command" bridge into a real `omp` runtime: spawn `omp` as a long-lived child process, stream its responses into the AI Chat panel, and host UnicDB's DB-aware tools into OMP's MCP bridge so the model can call them directly. Release target v1.11.0.

## §Constraints

- Base: `main @ 4a2c251` (cycle AD release v1.10.0).
- OMP availability = opt-in: detection already exists (`src/ai/omp/detect.ts`, `MIN_OMP_VERSION = "17.0.0"`). Activation gate checks `UnicDB.ai.engine === "omp"` and `omp --version ≥ 17.0.0`. Fallback to existing builtin engine + one-time install notice (per OMP-INTEGRATION-research.md §Recommendation).
- Privacy invariant (cycle AA): when UnicDB hosts the tools via the MCP bridge, the tools live inside the extension host — no schema/row bytes leave the extension except through `omp`'s configured base URL.
...
`AiSettings.engine: "builtin" | "omp"` (new field, default `"builtin"`). When the user picks `"omp"` (via the existing `UnicDB.ai.openAiSettings` form + a new UI element), the chat panel delegates to `ompChatEngine.send()` instead of the current `provider.ts`+`agent.ts` path.
...
3. `session/new` with `mcpServers: [{type:"http", name:"UnicDB", url:"http://127.0.0.1:<port>"}]` — verified acceptance per ACP-TOOLS-research.md.
...
- `src/extension.ts` — at activation, if `UnicDB.ai.engine === "omp"`, call `hostMcp.start()` and pass its port into `acpProcess` boot args.
...
0. **Detection gate**: with `UnicDB.ai.engine = "omp"` AND `omp` missing → builtin path runs + one-time install notice fires once per activation (idempotent across restarts).
...
7. **Cycle-AB regressions stay green**: image attach + clipboard paste still works in both engines; non-vision rejection still happens when `UnicDB.ai.models.work.vision = false` and engine is `omp` (mirror the cycle-AB omp gate).
...
- `npm run package -- --no-dependencies` → `UnicDB-1.11.0.vsix` with 18 entries, 0 forbidden, all v1.10.0 markers + new cycle AE markers (`ompChatEngine`, `hostMcp`, `UnicDB.ai.engine`)
- Manual smoke: spin VS Code, set `UnicDB.ai.engine = "omp"`, open AI Chat, send a query that requires DB context — observe streaming + a permission card for a DB tool, click Allow, see the inline tool result in the chat.
