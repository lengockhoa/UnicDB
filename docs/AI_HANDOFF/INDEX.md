...
|------|-------|--------|------|-------|----------|
| TASK-COLLAPSE-002 | Webview: enforce exactly 2-row toolbar — `.UnicDB-toolbar` column + two `.UnicDB-toolbar-row` wrappers; WHERE/ORDER BY `flex: 1 1 100%` in row 2; re-target transactionControls insertBefore; flip 4 test pins | approved_minor | none | webview/main.ts, webview/styles.css, src/ui/__tests__/webviewToolbar.test.ts, tests/webviewRequeryAlignment.test.ts, src/ui/__tests__/aiChatPanelCloneCss.test.ts, src/ui/__tests__/webviewRequery.test.ts | unic-smart |
