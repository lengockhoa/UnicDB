...
Goal: OMP runtime wiring — spawn omp as ACP child, host VSDB DB-aware tools via in-process MCP HTTP, stream responses into chat panel; release v1.11.0.
Tasks: 3 (T1 hostMcp.ts MCP HTTP server + tests, T2 ompChatEngine.ts chat glue + tests, T3 engine setting + chat panel routing + activation wiring + tests)
Status: plan written (PLAN_AE.md); pending wave 1 dispatch
