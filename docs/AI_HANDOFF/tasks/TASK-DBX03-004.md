...
Host service that fetches shapes + keyset-ordered rows and runs the pure diffs, a CSP-clean compare panel webview rendering schema diff / data diff / sync plan, and extension wiring for one new command `UnicDB.compareTables`.

## Target Files

- `src/ui/compareService.ts` — new host orchestration.
- `src/ui/comparePanel.ts` — webview host (panel creation, message channel `UnicDB-compare`, CSP nonce).
- `webview/comparePanelMain.ts` — webview script (renders sections from one postMessage payload).
- `src/extension.ts` — register `UnicDB.compareTables` behind the partial-mock guard pattern used by DBX-01/02 registrations.
- `package.json` — command entry (`UnicDB.compareTables`, category UnicDB, icon `$(diff)`).
...
| 6 | smoke | extension activation registers UnicDB.compareTables without throwing (partial mocks, mirrors dbx01Scaffold pattern) | registered command present | mocked vscode |
| 7 | scaffold | package.json has UnicDB.compareTables with category UnicDB + icon | manifest contains entry | pkg read |
...
```
- package.json contributes: command `UnicDB.compareTables` ("Compare Tables", category UnicDB, icon `$(diff)`); no new activation event (command palette runs post-activation).
