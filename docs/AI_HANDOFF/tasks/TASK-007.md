...
|---|---|---|---|---|
| 1 | happy (regression, P1-2) | Duplicate column sort posts the real column name | Sorting the second `id` column (`colId` `id__2`) posts exactly one `requery` whose `orderBy` is `id ASC` — the string does **not** contain `id__2`. RED today: `orderBy` is `id__2 ASC`. | Bundle + `__vsdb.debugSetSpecs([{field:"id",headerName:"id",kind:"number"},{field:"id__2",headerName:"id",kind:"number"}])`, then `applyColumnState` on `id__2`; postgres driver header |
...
| 3 | happy (regression, P1-3) | `ok:true` + warnings renders the banner | After `{type:"saveResult", index:0, ok:true, warnings:["row 1: ctid fallback is not safe under concurrent writes"]}`, `dom.saveBanner` is visible (no `vsdb-hidden` class, no `hidden` attribute) and its `textContent` contains `not safe under concurrent writes`. RED today: banner stays hidden. | Bundle + dispatched state with one row |
| 4 | edge — empty/boundary (P1-3) | `ok:true` with no warnings keeps the banner hidden | `{ok:true}` and `{ok:true, warnings:[]}` both leave `saveBanner` carrying `vsdb-hidden` and the `hidden` attribute, and `editState.dirtyCount` is `0`. | as #3, one dirty cell before the ack |
...
| 6 | edge — data loss / missing capability (regression, P2-5) | Refresh without `window.confirm` cannot silently discard | With `window.confirm` deleted from the jsdom window and `editState.dirtyCount === 1`, `__vsdb.refresh()` posts **zero** messages and `dirtyCount` stays `1`; the in-DOM confirm banner is visible. Clicking its Cancel control leaves `dirtyCount === 1` and posts nothing; clicking Discard sets `dirtyCount === 0` and posts exactly one `requery`. RED today: `dirtyCount` drops to 0 and a requery is posted on the first click. | Bundle + 3-row state + one simulated cell edit; `delete (window as any).confirm` |
| 7 | regression (P2-5) | Clean refresh is unchanged | With `dirtyCount === 0`, `__vsdb.refresh()` posts exactly one `requery` immediately and the confirm banner never becomes visible. | Bundle + 3-row state, no edits |
...
- `src/ui/__tests__/webviewFilters.test.ts` — case 5 (jsdom; `.vsdb-search-input` is queried the same way in `webviewBundle.test.ts:225`).
...
- Consumes (existing, unchanged — quoted from source):
  ...
  - `__vsdb` test seams: `debugSetSpecs(specs)`, `refresh`, `editState`, `gridApi`, `simulateCellEdit` (`webview/main.ts:3368-3437`).
- Produces:
