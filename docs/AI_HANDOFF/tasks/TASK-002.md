...
## Reviewer Verdict — cycle AE R1 [TASK-002] (unic-smart)
...
- FINDINGS (important):
  ...
  2. src/ai/omp/ompChatEngine.ts:216-231 — `tool_call` notification dispatches `hostMcp.call()`, but send() also hands omp the mcpServers HTTP descriptor (:258-263), so omp itself issues tools/call over HTTP (PLAN_AE §2/§3). Real wiring = every DB tool executes TWICE and prompts TWO permission cards (hostMcp.pending keyed per requestId; gatePost fires per execution). Fix: on `tool_call` fire onToolStart only; let omp's HTTP round-trip execute and use `tool_call_update` for the result — or drop mcpServers from the descriptor and make the engine the sole dispatch path (breaks §2; prefer the former).
  3. src/ai/omp/ompChatEngine.ts:296-298 — resume() replays absorbed history through dispatchNotification, whose tool_call branch calls hostMcp.call: a resumed session re-executes HISTORICAL DB queries and re-prompts permission for them (research doc: session/load replays incl. tool_call frames). Test only feeds an empty replay buffer, so this is untested. Fix: replay must render (onDelta/onThought only), never dispatch hostMcp.call.
  4. src/ai/omp/ompChatEngine.ts:270-272 — the registered notification handler stays live after send() resolves or onError fires; a late agent_message_chunk still fires onDelta into the next render — the exact orphan-bubble defect the raw path fixed with turnSettled (aiChatPanel.ts:943-947, Finding 1b). Fix: per-turn `settled` flag set in send()'s finally; dispatch returns early when settled.
  5. docs/AI_HANDOFF/tasks/TASK-002.md — no cycle-AE Executor Report in this file (EXECUTOR_MODEL/RED_OUTPUT/GREEN/COMMIT absent; only cycle-AB/AD history present) → incomplete handoff package per Quality Gate.
