...
- The host filters the list by `cwd`, excludes the current session, sorts by `updatedAt` descending, and caps at 20 (`RESUME_PICKER_CAP`, `src/ui/aiChatPanel.ts:1173`, applied at `src/ui/aiChatPanel.ts:4046`).
...
- Rows are capped at 20 — the existing `RESUME_PICKER_CAP` (`src/ui/aiChatPanel.ts:1173`).
...
| activity events | No timeline UI; trace ring is in-memory only | absent in current source (`src/ai/trace.ts:11`) |
...
| activity events | Trace recorder is not rendered | absent in current source (`src/ai/trace.ts:11`) |
