...
Archived from `docs/WORKLOG.md` on 2026-09-16 (CHATV2 close-out; source file was 751 lines / 57 entries, over the 600-line / 30-entry budget). August 2026 entries live here; September entries stay in the active file.
...
- Action: built real-time omp runtime on top of cycle AD's config bridge. Added `UnicDB.ai.engine` setting (builtin|omp), `src/ai/settings.ts` (engine field + aiSettingsErrors check + legacy migration), `src/ai/config.ts` (legacy migration), `src/extension.ts` (activation detect + routing), `src/ui/aiChatPanel.ts` (engine option + wiring), `package.json` (UnicDB-ai-engine activationEvent), new tests across `src/ai/omp/__tests__/`.
