...
//
// Lens command: "UnicDB.runStatement" với argument [statement, index].
// Filter: languageId === 'sql' AND UnicDB.showRunLens === true.
...
export class UnicDBCodeLensProvider implements vscode.CodeLensProvider {
   ...
   * every existing zero-arg `new UnicDBCodeLensProvider()` call (incl. all
  ...
  constructor(private readonly getDialect?: () => SqlDialect | undefined) {
    ...
    this.configSub = vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("UnicDB.showRunLens")) {
        this._onDidChangeCodeLenses.fire();
      }
      if (e.affectsConfiguration("UnicDB.showRunLensSh")) {
        this._onDidChangeCodeLenses.fire();
      }
    });
  }
  ...
  provideCodeLenses(
    ...
    // Shellscript lens: 1 lens ở line 0, command UnicDB.runScript, gated by showRunLensSh.
    if (document.languageId === "shellscript") {
      const cfgSh = vscode.workspace.getConfiguration("UnicDB");
