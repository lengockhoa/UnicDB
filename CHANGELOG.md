...
Cycle BQ-02: BigQuery Resource Explorer + Table Preview — BigQuery connections now expose their datasets, tables, views, columns and routines through the Schema Explorer, and a single click on a table node opens a cost-safe preview in the existing `vsdb.browseTableData` panel. The seam built in BQ-01 (`BigQueryAdapter` + `@google-cloud/bigquery@9.0.3`) is widened to the real client shapes (`getDatasets` / `dataset.getTables` / `table.getMetadata` / `createQueryJob` for the preview path); the BQ-00 frozen surface (`bigqueryTypes.ts`, `bigqueryAdc.ts`) is byte-untouched, and no driver besides `bigquery` gains new behavior. The cycle ships **+25 tests** over v1.48.0 (suite 3283 → 3308, floor preserved at 2 skipped).

### Added
- **Real BigQuery resource enumeration on `BigQueryAdapter`** (`src/adapters/bigquery.ts`, `src/adapters/__tests__/bigquery.test.ts`): `listSchemas` maps the underlying `getDatasets()` result so each BigQuery dataset surfaces as one schema entry (the Schema Explorer labels them "datasets", not "schemas" — naming follows the wire, not the cross-driver abstraction); `listTables`, `listViews`, `listColumns`, `listRoutines`, `listTableDetail` and `estimateTableRows` are real, all backed by client metadata calls (`dataset.getTables()` → `table.getMetadata()` for schema/column/type, `INFORMATION_SCHEMA.ROUTINES` for routine enumeration, `INFORMATION_SCHEMA.TABLE_STORAGE` / `__TABLES__` fallback for the bounded row estimate). The BQ-00 frozen surface (`bigqueryTypes.ts`, `bigqueryAdc.ts`) is byte-untouched. 8 new tests pin (1) datasets surface as schemas, (2) tables+views split correctly by table type metadata, (3) column metadata includes mode (NULLABLE / REQUIRED / REPEATED) and full type string, (4) routine enumeration reads from `INFORMATION_SCHEMA`, (5) `listTableDetail` carries schema + row estimate + size, (6) `estimateTableRows` is bounded and non-blocking on missing stats, (7) reconnect after disconnect re-fetches cleanly, and (8) the fake client signature matches the real `@google-cloud/bigquery@9.0.3` shape.
- **Bounded BigQuery preview SQL builder** (`src/adapters/bigqueryPreview.ts`, new; `src/adapters/__tests__/bigqueryPreview.test.ts`): pure `buildBigQueryPreviewSql({ projectId, datasetId, tableId, limit })` emits a 3-part qualified `SELECT … FROM \`${projectId}.${datasetId}.${tableId}\` LIMIT N` statement with `limit` clamped to the `[100, 1000]` window (the BQ-02 cost ceiling; below 100 is rare enough to be a UX surprise, above 1000 risks an unbounded bytes-billed surprise). Every project, dataset and table identifier is backtick-quoted with internal backtick doubling — untrusted identifiers cannot escape the reference. The wire SQL flows through the BQ-01 path: `runQuery` → `client.query({ query, skipParsing: true })` → `toBigQueryPage`, so `BigQueryInt64String` / `BigQueryNumericString` / `BigQueryBigNumericString` precision survives the preview round-trip. 6 new tests pin happy paths across all three identifiers, the limit clamp (below / within / above), backtick doubling in identifiers, and that the builder never embeds user values into the LIMIT.
- **Schema Explorer wiring for BigQuery** (`src/ui/schemaTree.ts`, `src/ui/__tests__/schemaTree.test.ts`): the existing schema tree accepts BigQuery datasets as schema nodes and BigQuery tables as table nodes. Tooltip on each table node carries the cost-safe shape: `bigquery@<billingProject>` plus `dataset.table` and the table type (BASE TABLE / VIEW / MATERIALIZED VIEW / EXTERNAL) — never an unredacted project id, never the SQL preview, never a row count. Row-count auto-refresh is suppressed for BigQuery tables in batch (BQ-02 cost rule: zero `INFORMATION_SCHEMA.TABLE_STORAGE` queries fire from the explorer refresh path; the explorer trusts `listTableDetail` and never issues its own row-count query). 7 new tests pin (1) bigquery icon, (2) `bigquery@<billingProject>` tooltip shape, (3) dataset-not-schema labeling, (4) zero row-count queries during explorer refresh, (5) view vs table rendering, (6) routine node rendering for BigQuery routines, (7) the table-click preview path fires only on bigquery tables (the existing pg/mysql/mssql click paths stay green).
- **Preview click path wired to `vsdb.browseTableData` for bigquery** (`src/ui/browseCommands.ts`, `src/core/connectionManager.ts`, `src/ui/__tests__/browseCommands.test.ts`): a table-node click for a bigquery adapter now invokes `vsdb.browseTableData` against the existing results panel using the bounded `buildBigQueryPreviewSql` output — the preview shows in the same panel a user already knows, with the same keyboard / load-more / cancel affordances as the pg/mysql/mssql previews. The new path is fully additive: existing pg/mysql/mssql preview wiring is byte-untouched. 4 new tests pin (1) the click dispatches the bounded SQL, (2) the result panel receives `BigQueryPage` rows with `BigQueryInt64String` precision intact, (3) the click path is no-op when the adapter is closed, and (4) an unfetched preview never blocks the existing load-more handler.

### Changed
- **BQ-00 frozen surface byte-untouched; seam widened to real `@google-cloud/bigquery@9.0.3` instance shapes** (`src/adapters/bigquery.ts`, `src/adapters/__tests__/bigquery.test.ts`): the BQ-00 modules (`bigqueryTypes.ts`, `bigqueryAdc.ts`) and the BQ-01 narrow `BigQueryClientLike` keep their byte-identical bodies; the new code paths (`listSchemas` / `listTables` / `listViews` / `listColumns` / `listRoutines` / `listTableDetail` / `estimateTableRows` / the preview SQL builder) call real `@google-cloud/bigquery@9.0.3` instance methods (`getDatasets`, `dataset.getTables`, `table.getMetadata`, `INFORMATION_SCHEMA.ROUTINES`, `INFORMATION_SCHEMA.TABLE_STORAGE`, `createQueryJob`) — the BQ-01 `clientFactory` seam accepts the wider real-client shape, and the existing `BigQueryClientFactory` test fake still matches because the method set is a strict superset. No behavior change for the pg / mysql / mssql adapters or any of the read-only guard / AI / console paths.
...
- **`listRoutineParams` still unimplemented**: routine enumeration lists names + return types; per-parameter inspection is documented as a known gap and pinned by a focused comment in the test (#4 above) so future cycles see the seam.
...
### Review
- P2.5 plan review: Approved by unic-smart (no blocking findings on the cycle plan; minors — backtick-doubling edge case noted and pinned, `INFORMATION_SCHEMA.ROUTINES` cross-region behavior noted and pinned — applied in this revision).
- R2 per-task review by unic-smart: 3/3 verdicts returned (BQ-02.001 `approved_minor`, BQ-02.002 `approved_minor`, BQ-02.003 `approved_minor`) — zero blocking findings, all deferred items above pinned by reviewers as in-scope deferrals (not unresolved threads).
- R4.5 fix loop: **not invoked** — all 3 verdicts were `approved_minor`; zero fix rounds needed.
- Verification: full suite **3308 passed | 2 skipped** (was 3283|2 at v1.48.0; +25 new tests; suite floor preserved); `npm run typecheck` + `npm run compile` exit 0; BQ-00 frozen surface diff empty (`git diff --stat -- src/adapters/bigqueryTypes.ts src/adapters/bigqueryAdc.ts` prints nothing); `package.json` diff is the version line only.
