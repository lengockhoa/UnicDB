...
function render(msg: ModelMessage): void {
  ...
  // Zoom: wheel scales the viewBox about its center. The webview is the
  // visual authority: it clamps locally against the model's natural
  // layout dimensions so scrolling can never exceed 0.25..4, and it
  // ignores any host ack that would reverse the gesture. Empty-layout
  // guard avoids NaN when base.w / base.h === 0.
  svg.addEventListener("wheel", (ev) => {
    ev.preventDefault();
    if (!current) return;
    const baseW = current.layout.width;
    const baseH = current.layout.height;
    if (baseW <= 0 || baseH <= 0) return;
    const currentScale = viewBox.w / baseW;
    const proposed = Math.min(4, Math.max(0.25, currentScale * (ev.deltaY < 0 ? 1 / 1.1 : 1.1)));
    const delta = proposed / currentScale;
    const w0 = viewBox.w;
    const h0 = viewBox.h;
    viewBox.w *= delta;
    viewBox.h *= delta;
...
window.addEventListener("message", (event: MessageEvent) => {
  ...
  // Host zoom acknowledgment — the webview's viewBox is the visual
  // authority (it clamps locally); the ack carries no geometry, so
  // nothing to reconcile. Kept as a no-op hook for future persistence.
});
